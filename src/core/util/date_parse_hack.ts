...
export function parseNote(s: string): string {
  let dateRelated = /^(\d{1,2}.\d{1,2}.\d{2,4}|\d{1,4}$|(янв)|(фев)|(мар)|(апр)|(мая)$|(май)$|(июн)|(июл)|(авг)|(сен)|(окт)|(ноя)|(дек)|(вчера)$|(позавчера)$|(сегодня)$|(завтра)$|(послезавтра)$|(\d{1,2}ч|\d{1,2} ч)|(в \d{1,2}:\d{1,2})$|(в\d{1,2}:\d{1,2})$|(\d{2} ми)|(\d{2}ми)|(\d{1,2} \d{2}м)|(в \d{1,2})$|(в\d{1,2})$|(\d{1,2}:\d{1,2})|дней$|лет$|нед|год$|мес$|месяц|день$|дня$|час$|часов$|часа$|полчаса$|мин$|минут|сек$|секунд|через$|назад$|(понед)|(вторн)|(сред)|(четв)|(пятн)|(субб)|(воскр))/;
